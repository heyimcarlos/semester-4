...
SELECT order_total
FROM (SELECT o.customer_id, SUM(oi.quantity * oi.unit_price) order_total
  FROM orders o
...
SELECT SUM(oi.quantity * oi.unit_price) -- SUM will return NULL if no rows found
FROM orders o
JOIN order_items oi ON o.order_id = oi.order_id
WHERE o.customer_id = 1;

SELECT o.order_id, oi.quantity, oi.unit_price
FROM orders o
JOIN order_items oi ON o.order_id = oi.order_id
...
select oi.quantity, oi.unit_price, oi.product_id, o.order_id
from orders o
join order_items oi ON o.order_id = oi.order_id
group by oi.quantity, oi.unit_price, oi.product_id, o.order_id
order by o.order_id;
-- get total amount of orders, and total purchases
...
BEGIN
  ...
  EXCEPTION
    WHEN NO_DATA_FOUND THEN
      DBMS_OUTPUT.PUT_LINE('No order found with Order ID: ' || v_order_id);
  END;
...
/* DECLARE
  v_spending_amount NUMBER := 10000;
...
BEGIN
  ...
  IF v_total_quantity <= 300 THEN
    v_shipping_cost := 50;
  ELSIF v_total_quantity <= 600 THEN
    v_shipping_cost := 75;
  ELSIF v_total_quantity <= 1000 THEN
    v_shipping_cost := 100;
  ELSE
    v_shipping_cost := 120;
  END IF;
  ...
  DBMS_OUTPUT.PUT_LINE('Shipping Cost: $' || v_shipping_cost);
END; */
